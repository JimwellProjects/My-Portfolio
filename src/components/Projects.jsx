...
const ProjectsCard = ({ icon: Icon, title, content }) => {
  ...
  useEffect(() => {
    if (open) {
      setTimeout(() => {
        const modalContent = document.querySelector('.react-responsive-modal-modal');
        if (modalContent) {
  ...
  return (
    <div className="bg-slate-900 rounded p-5">
      <img src={PROJECT_1_COVER} alt="Card Image" className="rounded mb-2 w-full h-48 object-cover" />
      ...
      <Modal open={open} onClose={onCloseModal} classNames={{ modal: 'bg-slate-50 p-6 rounded-lg shadow-lg size-10/12' }} center>
        <div className='p-4 overflow-y-auto'>
          <div className="flex flex-col items-center">
            <img src={PROJ_PIC_2} className="mb-4 rounded-lg size-11/12 max-w-full h-auto" alt="Project Image" />
